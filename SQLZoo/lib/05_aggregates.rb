...
## Example of aggregate functions: COUNT - num of rows in group, SUM(col), MAX(col), MIN(col), AVG(col)
def example_sum
	execute(<<-SQL)
		SELECT
			SUM(population)
...
def area_count
	...
	execute(<<-SQL)
		SELECT 
			COUNT(name) AS num_large_area_countries
		FROM 
			countries 
		WHERE 
			area > 1000000
...
def populous_country_counts
	...
	execute(<<-SQL)
		SELECT 
			continent, COUNT(name) AS num_large_populated_countries
		FROM
			countries 
		WHERE 
			population >= 10000000
