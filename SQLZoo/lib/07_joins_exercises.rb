	...
	execute(<<-SQL)
		...
		FROM
			...
			JOIN actors 
				ON actors.id = castings.actor_id
		WHERE 
			/* the following query returns list of films Julie Andrews played in */
			movies.id IN (
				SELECT
					movies.id
				FROM
		...
		/* leading actor condition is outside of subquery */
		castings.ord = 1
	SQL
end
...
def films_by_cast_size
	...
	execute(<<-SQL)
		...
		WHERE
			movies.yr = 1978
		GROUP BY 
		 	/* Did not get this, had to look at solutions, but it makes sense. In order to get number of actors in the cast, you need to GROUP BY movies.id, or movies.name, then count number of actors in cast */
			movies.id
		ORDER BY
